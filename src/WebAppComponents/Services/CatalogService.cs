...
public class CatalogService(HttpClient httpClient) : ICatalogService
{
    ...
    public async Task<CatalogResult> GetCatalogItems(
        int pageIndex,
        int pageSize,
        int? brand,
        int? type
    ...
    private static string GetAllCatalogItemsUri(
        string baseUri,
        int pageIndex,
        int pageSize,
        int? brand,
    ...
    public Task<ChatSessionDto?> GetChatSession(string sessionId, string userId)
    {
        var uri =
            $"{remoteServiceBaseUrl}chat/sessions/{sessionId}?userId={HttpUtility.UrlEncode(userId)}";
        return httpClient.GetFromJsonAsync<ChatSessionDto>(uri);
    }

    public async Task SaveChatSession(
        string sessionId,
        string userId,
        List<ChatMessageDto> messages
    )
    {
        var uri = $"{remoteServiceBaseUrl}chat/sessions";
