...
public static class CatalogApi
{
    public static IEndpointRouteBuilder MapCatalogApi(this IEndpointRouteBuilder app)
    {
        ...
        adminV1
            .MapPut("/items", UpdateItemV1)
            .WithName("UpdateItem")
            .WithSummary("Created or replace a catalog item")
            .WithDescription("Created or replace a catalog item")
            .WithTags("Items");

        adminV2
            .MapPut("/items/{id:int}", UpdateItem)
            .WithName("UpdateItem-V2")
            .WithSummary("Created or replace catalog item")
            .WithDescription("Created or replace catalog item")
            .WithTags("Items");

        adminApi
            .MapPost("/items", CreateItem)
            .WithName("CreateItem")
            .WithSummary("Created a catalog item")
            .WithDescription("Created a catalog item");

        adminApi
            .MapDelete("/items/{id:int}", DeleteItemById)
            .WithName("DeleteItem")
            .WithSummary("Delete catalog item")
            .WithDescription("Delete catalog item");
        ...
        //API Lưu Chat User cho AI
        api.MapGet("/chat/sessions/{sessionId}", GetChatSession)
            .WithName("GetChatSessions")
            .WithSummary("Get chat session history")
            .WithDescription("Get the chat session history session id")
            .WithTags("Chat");
        api.MapPost("/chat/sessions", SaveChatSession)
            .WithName("SaveChatSessions")
            .WithSummary("Save chat session history")
            .WithDescription("Save the chat session history")
            .WithTags("Chat");
        ...
        // Routes for modifying catalog items have been moved to the top of this method
        // and are now protected by the "AdminOnly" authorization policy.
    ...
    )
    {
        var items = await services
            .Context.CatalogItems.Where(item => ids.Contains(item.Id))
            .ToListAsync();
    ...
    public static async Task<
        Results<Ok<CatalogItem>, NotFound<ProblemDetails>, BadRequest<ProblemDetails>>
    > GetItemById(
    ...
    )
    {
        ...
        var item = await services
            .Context.CatalogItems.Include(ci => ci.CatalogBrand)
        ...
        if (item == null)
        {
            return TypedResults.NotFound<ProblemDetails>(
                new ProblemDetails
                {
                    ...
                    Detail = $"Catalog item with id {id} was not found.",
                }
            );
        }
    ...
    public static async Task<
        Results<Ok<PaginatedItems<CatalogItem>>, RedirectToRouteHttpResult>
    ...
    )
    {
        ...
        {
            var itemsWithDistance = await services
                .Context.CatalogItems.Where(c => c.Embedding != null)
    ...
    public static async Task<
        Results<Created, BadRequest<ProblemDetails>, NotFound<ProblemDetails>>
    > UpdateItemV1(
        HttpContext httpContext,
        [AsParameters] CatalogServices services,
    ...
    )
    {
        if (productToUpdate?.Id == null)
        {
            return TypedResults.BadRequest<ProblemDetails>(
                new() { Detail = "Item id must be provided in the request body." }
            );
        }
    ...
    )
    {
        ...
        {
            return TypedResults.NotFound<ProblemDetails>(
                new() { Detail = $"Item with id {id} not found." }
            );
        }
    ...
    public static async Task<Results<Ok<ChatSessionDto>, NotFound>> GetChatSession(
        [FromServices] IChatMemoryService chatMemoryService,
        [Description("The chat session id")] string sessionId,
        [Description("The user id"), FromQuery] string userId
    )
    {
        var session = await chatMemoryService.GetSessionAsync(sessionId, userId);
        if (session is null)
        {
            return TypedResults.NotFound();
        }
    ...
    public static async Task<Ok> SaveChatSession(
        [FromServices] IChatMemoryService chatMemoryService,
        [FromBody] SaveChatSessionRequest request
    )
    {
        await chatMemoryService.SaveSessionAsync(
            request.SessionId,
            request.UserId,
            request.Messages
        );
